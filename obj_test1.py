...
import threading
sys.path.append('..')
sys.path.append("/home/pi/tensorflow1/models/research/object_detection1")
from utils import label_map_util
from utils import visualization_utils as vis_util

##############For Rest Of the Objects#######################
# Name of the directory containing the object detection module we're using
def general_object_model():
...
#######################################################################333
#####################For Pen #####################################3
def pen_detection_model():
    MODEL_NAME1 = 'inference_graph'

    # Grab path to current working directory
    CWD_PATH1 = "/home/pi/tensorflow1/models/research/object_detection1"#os.getcwd()

    # Path to frozen detection graph .pb file, which contains the model that is used
    # for object detection.
    PATH_TO_CKPT1 = os.path.join(CWD_PATH1,MODEL_NAME,'frozen_inference_graph.pb')

    # Path to label map file
    PATH_TO_LABELS1 = os.path.join(CWD_PATH1,'training','labelmap.pbtxt')

    # Number of classes the object detector can identify
    NUM_CLASSES1 = 1
    label_map1 = label_map_util.load_labelmap(PATH_TO_LABELS1)
    categories1 = label_map_util.convert_label_map_to_categories(label_map1, max_num_classes=NUM_CLASSES1, use_display_name=True)
...
s1=threading.Thread(general_object_model)
s2=threading.Thread(pen_detection_model)
s1.deamon=True
s2.deamon=True
s1.start()
s2.start()
